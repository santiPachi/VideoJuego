...
using UnityEngine;
    ...
    public class CarController : MonoBehaviour
    {
        ...
        private float OriginalWheelSteerR;
        ...
        public void TurboPartile(){
            if(turboParticle != null){
                if(isTurbo){
                     turboParticle.Emit(1);
                }else{
                     turboParticle.Stop();
                }
            }
        ...
        public void Move(float steering, float accel, float footbrake, float handbrake)
        {
            ...
            if(CurrentSpeed <= MaxSpeed  || isTurbo || accel == 0){
            ...
            if (handbrake > 0f)
            {
                ...
                lightManager.ToggleTailLights(true);
            }else{
                m_WheelColliders[2].brakeTorque = 0f;
                m_WheelColliders[3].brakeTorque = 0f;
                lightManager.ToggleTailLights(false);
            }

            

            lightManager.ToggleBackLights(accel);
            CalculateRevs();
            GearChanging();
            ...
            TractionControl();
             CarLoopHelp();
            Speepometer.ShowSpeed(CurrentSpeed,m_Topspeed);
        }
        int count = 0;
